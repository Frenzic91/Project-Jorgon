class Entities {
  ...
  drawItems(playerX, playerY){
    ...
    let EndY = startY + HEIGHT/TILESIZE + 1; // + 1 to prevent items disappearing when heading up off screen
    ...
    for(let i = getIndexFromCoords(startX,startY); i <= getIndexFromCoords(EndX,EndY); i++){
      if(tileData[i].itemStack.length > 0){
        for (let j = 0; j < tileData[i].itemStack.length; j++) {
          //console.log("bruh");
            this.canvas.drawImage(getImageByIndex(itemImg["item"][tileData[i].itemStack[j].id],0), tileData[i].x * TILESIZE - TILESIZE/2, tileData[i].y * TILESIZE - TILESIZE/4);
        }
      }
    }
  }
